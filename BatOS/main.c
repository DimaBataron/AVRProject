 ...
 * Author : dima
 //Использовать директивы условной компилции для добавления в модули частей кода, добавленных модулей
 //Предусмотреть подключение многих энкодеров. и многих генераторов частоты
 при разводке платы
 //Процедуру вывода одного символа. Далее( без указания строки столбца)
 
 //Процедуры аварийного прикращения работы при переполнении очередей
 // Либо ожидание освобождения очереди после чего повтороить.
 // В протеус не работает часть кода с атомнарными операциями почему?
 // Сделать чтоб заработало
 // Timer 0 на этом же выводе PD4 может быть дело в этом
 */ 
//#include "BatOS.h"
#include "BatIO.h"
...
static volatile unsigned long  FStep=0;		// Величина шага изменения которую выбрали
static volatile unsigned int  GenFreq=0; // Частота которую накрутили выводится на экран
static volatile unsigned long OutFreq=0; // число которое отправляем в микросхему unsigned
static volatile unsigned long Mng=0; // множитель для вычисления 
...
int main(void)
{
	...
	// проверить повторную постановку задач в очередь,  после очистки очереди.
...
//Написать процедуры стартовой инициализации памяти 0xFF?? А надо ли??
...
//0 вложенность (частота  режим) 

//1 вложенность (шаг) (кручу сначала по 5, 20, 50, 200, 1000, 5000гЦ 
// 50кГц, 200кЦ, 1мГц)
// При коротком нажатии меняются эти 2 режима. При длительном выход в 0 вложенность

//2 вложенность (режим).Короткое нажатие выбор режима и переход в вложенность 1.
//длительное нажатие выход в вложенность 0
...
//===========================================================================================
	...
	if(OldNest==0){ //сделано короткое нажатие в меню вложенность 1;
		...
		// сюда зашли впервые по-любому
			BPrintf('s',si,5,3); //и т.д.
			BPrintf('s',tr,4,3);
			BPrintf('p',0,5,2); //вывод указателя в начало
		}
		else{ //режим выбора частоты УКАЗАТЕЛЬ ТУТ НЕ ИСПОЛЬЗУЕТСЯ
	...
	if(Nesting==1){ //скорее всего текст меню уже вывели(перерисовываем указатель)
